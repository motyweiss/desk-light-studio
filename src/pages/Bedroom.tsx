...
const Bedroom = () => {
  ...
  const [bedsideLampIntensity, setBedsideLampIntensity] = useState(0);
  const [ceilingFanLightIntensity, setCeilingFanLightIntensity] = useState(0);
  const [closetLightIntensity, setClosetLightIntensity] = useState(0);
  ...
  const allLightsOn = bedsideLampIntensity > 0 && ceilingFanLightIntensity > 0 && closetLightIntensity > 0;
  const anyLightOn = bedsideLampIntensity > 0 || ceilingFanLightIntensity > 0 || closetLightIntensity > 0;

  const handleMasterToggle = (checked: boolean) => {
    const targetValue = checked ? 100 : 0;
    setBedsideLampIntensity(targetValue);
    setCeilingFanLightIntensity(targetValue);
    setClosetLightIntensity(targetValue);
  };
  ...
  const lights = useMemo(() => [
    {
      id: 'bedside',
      label: 'Bedside Lamp',
      intensity: bedsideLampIntensity,
      onChange: setBedsideLampIntensity,
      isLoading: false,
    },
    {
      id: 'ceiling',
      label: 'Ceiling Fan Light',
      intensity: ceilingFanLightIntensity,
      onChange: setCeilingFanLightIntensity,
      isLoading: false,
    },
    {
      id: 'closet',
      label: 'Closet Light',
      intensity: closetLightIntensity,
      onChange: setClosetLightIntensity,
      isLoading: false,
    },
  ], [bedsideLampIntensity, ceilingFanLightIntensity, closetLightIntensity]);
    ...
    <>
      ...
      >
        <div className="relative z-10 flex flex-col md:flex-row items-start justify-center gap-8 md:gap-12 lg:gap-16 w-full max-w-6xl mx-auto px-6 py-10 md:py-20 min-h-[100dvh]">
          ...
          >
            <DeskDisplay 
              spotlightIntensity={bedsideLampIntensity}
              deskLampIntensity={ceilingFanLightIntensity}
              monitorLightIntensity={closetLightIntensity}
              hoveredLightId={hoveredLight}
              isLoaded={isLoaded}
              onSpotlightChange={setBedsideLampIntensity}
              onDeskLampChange={setCeilingFanLightIntensity}
              onMonitorLightChange={setClosetLightIntensity}
            />
          </motion.div>
