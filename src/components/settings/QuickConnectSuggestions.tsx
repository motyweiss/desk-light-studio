...
const COMMON_HA_URLS = [{
  url: "https://YOUR-ID.ui.nabu.casa",
  label: "Nabu Casa Cloud",
  icon: Cloud,
  isTemplate: true
}, {
  url: "http://homeassistant.local:8123",
  label: "homeassistant.local",
  icon: Globe
}, {
...
const saveRecentUrl = (url: string) => {
  ...
  const updated: RecentUrl[] = [{
    ...
    label,
    lastUsed: Date.now()
  }, ...filtered].slice(0, MAX_RECENT_URLS);
...
export const QuickConnectSuggestions = ({
  ...
  return <div className="space-y-4">
      {/* Recent Connections */}
      {recentUrls.length > 0 && <div className="space-y-2">
          ...
          return <motion.button key={url} onClick={() => testUrl(url, false)} disabled={!accessToken || testingUrl !== null} className={`
                    group relative flex items-center gap-2 px-3 py-2 rounded-lg text-sm
                    transition-all duration-200 disabled:cursor-not-allowed
                    ${isActive ? 'bg-green-500/20 border border-green-500/30 text-green-200' : 'bg-amber-500/10 border border-amber-500/20 text-amber-200 hover:bg-amber-500/20'}
                  `} whileHover={{
            scale: 1.02
          }} whileTap={{
            scale: 0.98
                  ...
                  {/* Remove button */}
                  <button onClick={e => handleRemoveRecent(e, url)} className="opacity-0 group-hover:opacity-100 ml-1 p-0.5 rounded hover:bg-white/10 transition-opacity">
                    <X className="w-3 h-3" />
                  </button>

                  <AnimatePresence mode="wait">
                    {isTesting && <motion.div initial={{
                opacity: 0,
                scale: 0.8
              }} animate={{
                opacity: 1,
                ...
                scale: 0.8
                    ...
                    {!isTesting && result === 'success' && <motion.div initial={{
                opacity: 0,
                scale: 0.8
              }} animate={{
                opacity: 1,
                ...
                scale: 0.8
                    ...
                    {!isTesting && result === 'failed' && <motion.div initial={{
                opacity: 0,
                scale: 0.8
              }} animate={{
                opacity: 1,
      ...
      {!accessToken && <p className="text-xs text-amber-400/70 flex items-center gap-1.5">
